package com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise

import androidx.compose.foundation.layout.fillMaxSize
import androidx.compose.foundation.layout.fillMaxWidth
import androidx.compose.foundation.layout.height
import androidx.compose.material3.Scaffold
import androidx.compose.runtime.*
import androidx.compose.ui.Modifier
import androidx.compose.ui.tooling.preview.Preview
import androidx.compose.ui.unit.dp
import androidx.navigation3.runtime.NavBackStack
import androidx.navigation3.runtime.NavKey
import androidx.navigation3.runtime.entryProvider
import androidx.navigation3.runtime.rememberNavBackStack
import androidx.navigation3.ui.NavDisplay
import androidx.savedstate.serialization.SavedStateConfiguration
import com.example.myfirstkmp.migration.session5.exercises.HomeScreen
import com.example.myfirstkmp.migration.session5.exercises.WelcomeScreen
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.authflow.forgotpassword.ForgotPasswordScreen
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.authflow.login.LoginScreen
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.authflow.register.RegisterScreen
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.base.BaseBottomBar
import com.example.myfirstkmp.migration.session2.exercises.DashboardScreen
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.navigation.AppFlow
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.navigation.AuthFlow
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.navigation.Back
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.navigation.Discover
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.navigation.ForgotPassword
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.navigation.Home
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.navigation.Login
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.navigation.Profile
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.navigation.Register
import com.example.myfirstkmp.migration.session5.exercises.authtabflowexercise.navigation.navbarItems
import com.example.myfirstkmp.theme.AppTheme
import kotlinx.serialization.ExperimentalSerializationApi
import kotlinx.serialization.modules.SerializersModule
import kotlinx.serialization.modules.polymorphic

/**
 * ⭐⭐⭐⭐ BÀI TẬP 4: Auth + Tab App (Advanced — 120 phút)
 *
 * Pattern này xuất hiện trong HẦU HẾT production apps.
 *
 * Yêu cầu:
 * - Auth flow: LoginKey → RegisterKey → ForgotPasswordKey
 * - Main flow: 3 tabs (Feed, Discover, Profile) với per-tab stacks
 * - `isAuthenticated: Boolean` state ở App level
 * - Login success → isAuthenticated = true → hiện Main flow (Auth stack bị unmount)
 * - Logout → isAuthenticated = false → hiện Auth flow
 * - Back từ Main KHÔNG về Login sau khi đã login
 * - BackHandler trên ForgotPasswordKey: "Bạn có chắc muốn quay lại?"
 * - LaunchedEffect khi vào ProfileKey: giả lập fetch user data (delay + update state)
 */

fun handleNavigateAuthFlow(destination: AuthFlow, backStack: NavBackStack<NavKey>) {
    when (destination) {
        is Login -> {
            if (backStack.contains(Login)) {
                while (backStack.last() is Login) {
                    backStack.removeLastOrNull()
                }
            }
        }

        is Register -> {
            backStack.add(Register)
        }

        is ForgotPassword -> {
            backStack.add(ForgotPassword)
        }

        is Back -> {
            if (backStack.size > 1) backStack.removeLastOrNull()
        }
    }
}

@OptIn(ExperimentalSerializationApi::class)
private val config = SavedStateConfiguration {
    serializersModule = SerializersModule {
        polymorphic(NavKey::class) {
            subclassesOfSealed<AuthFlow>()
            subclassesOfSealed<AppFlow>()
        }
    }
}

@Composable
fun AuthFlow(
    onAuthenticated: (Boolean) -> Unit
) {
    val backstack = rememberNavBackStack(configuration = config, elements = arrayOf(Login))

    val navigation = { destination: AuthFlow ->
        handleNavigateAuthFlow(destination, backstack)
    }

    NavDisplay(
        backStack = backstack,
        entryProvider = entryProvider {
            entry<Login> {
                LoginScreen(
                    onAction = { action ->
                        onAuthenticated(true)
                    },
                    onNavigate = navigation
                )
            }

            entry<Register> {
                RegisterScreen(onNavigate = navigation)
            }

            entry<ForgotPassword> {
                ForgotPasswordScreen(onNavigate = navigation)
            }
        }
    )
}

@Composable
private fun SetupNavigation(
    backstack: NavBackStack<NavKey>
) {

    NavDisplay(
        modifier = Modifier.fillMaxSize(),
        backStack = backstack,
        entryProvider = entryProvider {
            entry<Home> {
                WelcomeScreen()
            }

            entry<Discover> {
                HomeScreen()
            }

            entry<Profile> {
                DashboardScreen()
            }
        }
    )
}

@Composable
private fun AppFlow() {
    val homeBackstack = rememberNavBackStack(config, elements = arrayOf(Home))
    val discoverBackstack = rememberNavBackStack(config, elements = arrayOf(Discover))
    val profileBackstack = rememberNavBackStack(config, elements = arrayOf(Profile))

    var curBackstack by remember { mutableStateOf(homeBackstack) }

    Scaffold(
        modifier = Modifier.fillMaxSize(),
        bottomBar = {
            BaseBottomBar(
                modifier = Modifier
                    .fillMaxWidth()
                    .height(80.dp),
                items = navbarItems,
                isSelected = selected@{ pos ->
                    val firstEntry = when (pos) {
                        0 -> Home
                        1 -> Discover
                        2 -> Profile
                        else -> throw Exception()
                    }
                    return@selected curBackstack.contains(firstEntry)
                },
                onItemSelected = { pos ->
                    curBackstack = when (pos) {
                        0 -> homeBackstack
                        1 -> discoverBackstack
                        2 -> profileBackstack
                        else -> throw Exception()
                    }
                }
            )
        }
    ) { _ ->
        SetupNavigation(curBackstack)
    }
}

@Composable
fun AuthTabApp() {
    var isAuthenticated by remember { mutableStateOf(false) }

    if (!isAuthenticated) {
        AuthFlow(
            onAuthenticated = {
                isAuthenticated = it
            }
        )
        return
    }

    AppFlow()
}

@Preview(showBackground = true)
@Composable
private fun AuthTabAppPreview() {
    AppTheme { AuthTabApp() }
}
