...
package com.example.myfirstkmp.migration.session5.exercises.ecomerceflowexercise
...
import com.example.myfirstkmp.formatString
...
@Serializable
sealed interface CategoryFlow : NavKey
    ...
    Column(
        ...
        LazyColumn(
            ...
            items(items = categories, key = { item -> item }) { item ->
                CategoryItem(
                    title = item,
                    onClick = { onNavigate(ListProduct) }
                )
            }
        }
    }
...
fun ProductRow(
    ...
    Column(
        ...
        Row(
            ...
            Text(
                text = formatString(product.price),
    ...
    Column(
        ...
        LazyColumn(
            ...
            items(products) { product ->
                ProductRow(product = product) { onNavigate(ProductDetail(product.id)) }
            }
        }
    }
}
...
fun CartItemRow(
    ...
    Column(
        ...
        Row(
            ...
            Text(
                text = formatString(item.product.price),
                color = Color(0xFF6A4DF7),
                fontWeight = FontWeight.Bold
            )
        }
...
fun handleNavigation(
    ...
    when (intent) {
        ...
        is ListProduct -> backStack.add(ListProduct)

        is ProductDetail -> backStack.add(ProductDetail(intent.productId))

        is Cart -> backStack.add(Cart)
...
private val config = SavedStateConfiguration {
    serializersModule = SerializersModule {
        polymorphic(NavKey::class) {
            subclassesOfSealed<CategoryFlow>()
        }
    }
}
...
fun ECommerceApp() {
    ...
    val backStack = rememberNavBackStack(config, Category)
